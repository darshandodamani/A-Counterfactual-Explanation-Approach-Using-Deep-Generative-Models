...
def vae_loss_mse(x, recon_x, mu, logvar, kl_weight=1.0):
    recon_loss = mse_loss_fn(x, recon_x, reduction="sum")
    kld = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    return recon_loss + kl_weight * kld

# We can choose loss type: "logcosh" or "mse"
loss_type = "mse"

def vae_loss_selected(x, recon_x, mu, logvar, kl_weight=1.0):
    if loss_type == "logcosh":
        recon_loss = safe_log_cosh_loss(x, recon_x, a=1.0, reduction="sum")
    elif loss_type == "mse":
...
###############################################
def save_reconstruction_sample(model, dataloader, epoch, device, folder="plots/reconstructions"):
    model.eval()
...
def validate_epoch(model, valloader, kl_weight, device):
    ...
    return avg_loss, avg_recon, avg_kl, avg_acc

###############################################
# Plot Metrics and Save as Separate Files
###############################################
def plot_metrics(train_losses, val_losses, train_recon_losses, val_recon_losses, train_kl_losses, val_kl_losses, val_accuracies, num_epochs):
    ...
    plt.gca().yaxis.set_major_formatter(mtick.FormatStrFormatter('%.2f'))
    plt.tight_layout()
    plt.savefig("plots/recon_loss.png")
    plt.close()
    ...
    print("Metric plots saved in the 'plots' folder.")
...
def main():
    ...
    csv_file = os.path.join("logs", "training_log.csv")
    
    # Write CSV header
    with open(csv_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Epoch", "Train Loss", "Train Recon Loss", "Train KL Loss", "Val Loss", "Val Recon Loss", "Val KL Loss", "Val Pixel Accuracy", "KL Weight"])
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        val_loss, val_recon, val_kl, val_acc = validate_epoch(model, val_loader, kl_weight, device)
        ...
        with open(csv_file, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([epoch, train_loss, train_recon, train_kl, val_loss, val_recon, val_kl, val_acc, kl_weight])
        
        # Save sample reconstruction every 10 epochs.
        if epoch % 10 == 0:
            save_reconstruction_sample(model, val_loader, epoch, device, folder="plots/reconstructions")
    ...
    plot_metrics(train_losses, val_losses, train_recon_losses, val_recon_losses, train_kl_losses, val_kl_losses, val_accuracies, epoch)
